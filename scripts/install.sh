...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --help|-h)
            ...
            printf "    --domain DOMAIN        Public domain, e.g. consent.example.com (sets APP_URL)\n"
...
clone_repo() {
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            # Reset to latest remote — safe because users don't modify source files
            run_with_spinner "Fetching latest changes" git fetch origin "$BRANCH"
            git reset --hard "origin/$BRANCH" > /dev/null 2>&1 \
                || fatal "Failed to update. Remove the directory and reinstall."
            return 0
        else
...
configure_env() {
    ...
    if [ -f .env ]; then
        warn ".env already exists — keeping your configuration"
        apply_domain
        return 0
    fi
    ...
    SCANNER_API_KEY=$(generate_secret | head -c 32)
    ...
    sed \
        ...
        -e "s|^SCANNER_API_KEY=.*|SCANNER_API_KEY=${SCANNER_API_KEY}|" \
...
# ── Apply --domain to .env ───────────────────────────────────
#
# Sets APP_URL so generated consent scripts point at the right host.
# Existing installs additionally need `scripts:regenerate` — that runs
# in start_services() once the containers are up.

apply_domain() {
    [ -n "$DOMAIN" ] || return 0

    # Accept bare domains and full URLs. A bare domain implies https (the
    # sane default for a public install); an explicit http:// is respected,
    # so LAN-only setups aren't forced onto TLS they don't have.
    _scheme="https"
    case "$DOMAIN" in
        http://*) _scheme="http" ;;
    esac
    _host=$(printf '%s' "$DOMAIN" | sed -e 's|^https\{0,1\}://||' -e 's|/.*$||')
    if [ -z "$_host" ]; then
        fatal "Invalid --domain value: $DOMAIN"
    fi
    APP_URL_VALUE="${_scheme}://${_host}"

    if grep -qE '^APP_URL=' .env; then
        sed -i.bak -e "s|^APP_URL=.*|APP_URL=${APP_URL_VALUE}|" .env && rm -f .env.bak
    else
        printf '\nAPP_URL=%s\n' "$APP_URL_VALUE" >> .env
    fi

    success "APP_URL set to ${APP_URL_VALUE}"
    info "Terminate TLS for ${_host} in front of Conzent — see docs/custom-domain.md"
}
...
start_services() {
    ...
    if [ "$IS_FRESH" = true ]; then
        ...
        # Build and up are separate because older docker compose doesn't support --no-cache on up
    ...
    # Anonymous telemetry — report install/update to central tracker
    _ping_event="install"
    if [ "$UPDATE_MODE" = true ] || [ "$EXISTING_INSTALL" = true ]; then
        _ping_event="update"
    fi
    curl -sSL -o /dev/null "https://app.getconzent.com/ping?e=${_ping_event}&v=1" < /dev/null 2>/dev/null || true
    ...
    # Register the bundled scanner (idempotent upsert keyed by URL)
    run_with_spinner "Registering cookie scanner" \
        compose exec -T app php bin/oci scanner:register || \
        warn "Scanner registration failed — register it later with: docker compose exec app php bin/oci scanner:register"

    # A custom domain is baked into every generated consent script, so any
    # existing scripts must be rewritten against the new APP_URL.
    if [ -n "$DOMAIN" ] && [ "$IS_FRESH" = false ]; then
        run_with_spinner "Regenerating consent scripts for new domain" \
            compose exec -T app php bin/oci scripts:regenerate || \
            warn "Script regeneration failed — run: docker compose exec app php bin/oci scripts:regenerate"
    fi
...
print_success() {
    ...
    # A configured public domain wins over the loopback address
    CONFIGURED_URL=$(grep -E '^APP_URL=' .env 2>/dev/null | cut -d= -f2- || echo "")
    ...
    printf "    ${DIM}bash scripts/backup.sh${RESET}            Back up database + consent scripts\n"
    printf "\n"
    printf "  ${BOLD}Next steps:${RESET}\n"
    printf "    ${DIM}docs/custom-domain.md${RESET}    Point your own domain at this install\n"
    printf "    ${DIM}docs/credentials.md${RESET}      Rotate the generated passwords\n"
    printf "    ${DIM}docs/backup-restore.md${RESET}   Set up backups before going live\n"
    printf "\n"
