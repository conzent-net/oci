  ...
  (() => {
      ...
      currentLang: (function() {
        ...
        if (hreflangs.length > 0) {
          ...
          for (var i = 0; i < hreflangs.length; i++) {
            var href = (hreflangs[i].getAttribute('href') || '').replace(/[#?].*$/, '').replace(/\/+$/, '');
    ...
    const ConzentFN = function (event) {
      ...
      if (current_settings.close_button_status === 1) {
        cookieNotice = cookieNotice.replaceAll("[conzent_close_button]", closeIcon);
      } else {
      ...
      setTimeout(() => {
        ...
        function _cnzRenderCookieTables(selector) {
          ...
          current_settings.cookieTypes.forEach(function(field) {
            var el = _QE(prefix + "#cnzCategory" + field.value + " .conzent-cookie-table");
            if (!el) return;
            if (current_settings.cookiesList.hasOwnProperty(field.value)) {
              var cookieTbl = "";
              current_settings.cookiesList[field.value].forEach(function(item) {
                var desc = (item.description && item.description[currentLang]) || "";
                var dur = (item.duration && item.duration[currentLang]) || "";
                cookieTbl +=
                  '<ul class="cookie-items"><li><div class="item-name">' +
                  current_settings.shortCodes["conzent_cookie_list_cookie"][currentLang] +
                  '</div><div class="item-value">' + item.name +
                  '</div></li><li><div class="item-name">' +
                  current_settings.shortCodes["conzent_cookie_list_description"][currentLang] +
                  '</div><div class="item-value">' + desc +
                  '</div></li><li><div class="item-name">' +
                  current_settings.shortCodes["conzent_cookie_list_duration"][currentLang] +
                  '</div><div class="item-value">' + dur +
                  "</div></li></ul>";
              });
              ...
              el.innerHTML =
                ...
                current_settings.shortCodes["conzent_cookie_list_no_cookie_to_display_text"][currentLang] +
                "</p>";
            }
          });
        }
        ...
        function _cnzFetchCookies() {
          ...
          current_settings.cookieTypes.forEach(function(field) {
            var el = _QE("#cnzCategory" + field.value + " .conzent-cookie-table");
            ...
            if (xhr.readyState === 4) {
              if (xhr.status === 200) {
                try {
                  ...
                  if (_QE(".cnz-cookie-table")) _cnzRenderCookieTables(".cnz-cookie-table ");
          ...
          xhr.open("GET", "[API_PATH]/cookies?website_id=[WEBSITE_KEY]&_=" + Date.now(), true);
          xhr.send();
        }

        if (current_settings.cookiesList && Object.keys(current_settings.cookiesList).length > 0) {
          _cnzRenderCookieTables("");
          _cnzCookiesLoaded = true;
        ...
        if (CNZ_config._fw && CNZ_config._fw.bs === "block_none" && event !== "open") {
          _cnzDebug("hide", "ConzentFN: block_none framework — hiding banner, showing revisit icon for opt-out access");
          if (_QE("#Conzent")) {
    ...
    function setup_gcm_init() {
      if (CNZ_config.settings.google_consent != 1) return;
      ...
      window.dataLayer.push({ event: "conzent_loaded" });

      // Set consent defaults ONLY when GTM has not already loaded.
      // If GTM is present, a CMP template likely set region-specific
      // defaults (e.g. EEA=denied, US=granted) — don't overwrite them.
      if (isGtmLoaded() != true) {
    ...
    function setup_gcm_update() {
      ...
      window.dataLayer.push({ event: "conzent_consent_update" });
    ...
    function _resolveFrameworkRules() {
      ...
      if (rules) {
        ...
        if (rules.bs === "block_none") {
          ...
          var _allCats = ["functional", "analytics", "marketing", "preferences"];
          for (var _ac = 0; _ac < _allCats.length; _ac++) {
            if (CNZ_config.settings && CNZ_config.settings.allowed_categories &&
    ...
    function _showBanner() {
      ...
      if (CNZ_config.settings.geo_target == "selected") {
        ...
        if (CNZ_config.in_eu == true) {
          show_banner = 1;
          _cnzDebug("show", "Geo targeting: visitor country '" + CNZ_config.currentTarget + "' is in EU/UK");
        } else {
          show_banner = 0;
          _cnzDebug("hide", "Geo targeting: visitor country '" + CNZ_config.currentTarget + "' is NOT in EU/UK");
        }
      } else {
      ...
      if (!_geoExplicit && CNZ_config.currentTarget && _cnzFrameworkRules &&
        ...
        _cnzDebug("info", "Framework rules: skipped — explicit geo targeting overrides");
    ...
    function loadInit() {
      ...
      if (CNZ_config._autoGranted && CNZ_config._fw && CNZ_config._fw.bs === "block_none") {
        var _reGrantCats = ["functional", "analytics", "marketing", "preferences"];
        for (var _rg = 0; _rg < _reGrantCats.length; _rg++) {
          if (!CNZ_config.settings.allowed_categories.includes(_reGrantCats[_rg])) {
      ...
      if (preferences_selected || ConsentExists) {
        ...
        var _grantCats = ["functional", "analytics", "marketing", "preferences"];
        for (var _gi = 0; _gi < _grantCats.length; _gi++) {
          if (!CNZ_config.settings.allowed_categories.includes(_grantCats[_gi])) {
