	/*!
	 * Conzent CMP — IAB TCF engine.
	 *
	 * VENDORED, HAND-PATCHED BUILD. There is no build source in this repo:
	 * no package.json, no node_modules, no bundler config, no sourcemap.
	 * A prebuilt @iabtcf/core + @iabtcf/cmpapi + Preact bundle was committed
	 * wholesale and has been edited by hand ever since. Do NOT regenerate or
	 * re-vendor it without re-applying the patches below, and do not run it
	 * through a minifier — ScriptGenerationService injects this file AFTER
	 * minification via a string token precisely so it ships verbatim.
	 *
	 * Hand-applied patches:
	 *   - Null guards around googleVendors / #cnzIABTabVendor (commit db7e7341).
	 *   - {vendor_count} token replacement for the notice text.
	 *   - Explicit `segments: [CORE, VENDORS_DISCLOSED, PUBLISHER_TC]` at the
	 *     dn.encode() call. REQUIRED: TCF v2.3 makes DisclosedVendors mandatory
	 *     in every TC string, and the stock SegmentSequence omits it for
	 *     service-specific CMPs. Removing it silently breaks compliance.
	 *   - TCF 2.4 `standardTexts` support (search "TCF 2.4"): the bundled GVL
	 *     class predates the field and would otherwise drop it on populate().
	 *
	 * Verify with `node --check` after any edit.
	 */
	var n, s, e, i, o, r, a, p = {},
		l = [],
	...
	function _et() {
			...
			function _tt() {
				var e = {
					...
					t = {},
					// The old code did cz._addtlConsent.split("~")[1].split("."),
					// which throws on any value without a "~" and takes the whole
					// modal render down with it — reachable via a corrupted or
					// foreign euconsent-v2 cookie. _acParse handles v1 and v2.
					g = (cz._tcModal && cz._tcModal.gvl && cz._tcModal.gvl.googleVendors) || {},
					n = k2(g);
				return (cz._acConsented || cz._acParse(cz._addtlConsent)).forEach((function(e) {
					t[e] = !0
				})), Object.keys(g).forEach((function(r) {
					t[r] ? e.consent.allowed.push(r) : e.consent.rejected.push(r)
				})),
				// n > 0 guard: k2({}) is 0, so an empty vendor list used to render
				// the master toggle checked with nothing behind it. mapGoogleVendors
				// produces {} whenever the ATP list fails to load.
				e.sectionChecked = n > 0 && e.consent.allowed.length === n, e
			}
	...
	class pn extends ze {
		...
		features;
		specialFeatures;
		// TCF 2.4: top-level GVL object holding IAB's standard explanation
		// texts (currently just `features`). Undefined on pre-2.4 lists.
		standardTexts;
		isReady_ = !1;
		vendors_;
		vendorIds;
		...
		cacheLanguage() {
			pn.LANGUAGE_CACHE.has(this.cacheLang_) || pn.LANGUAGE_CACHE.set(this.cacheLang_, {
				...
				// TCF 2.4: translated per language, so it must survive a
				// LANGUAGE_CACHE hit in changeLanguage().
		...
		async fetchJson(t) {
			// Hard 5s deadline (the upstream default was 0 = wait forever, so a
			// hanging origin blocked CMP boot indefinitely) plus a network-first
			// localStorage fallback: only a full vendor list is ever cached, and
			// it is only served when the network fetch failed — a stale vendor
			// list beats no banner. Translation files pass through untouched.
			try {
				const e = await an.fetch(t, !1, 5e3);
				if (this.populate(e), void 0 !== e.vendors) try {
					localStorage.setItem("cnz_gvl", JSON.stringify({ u: t, t: Date.now(), d: e }))
				} catch (t) {}
			} catch (e) {
				let n = null;
				try {
					const t = localStorage.getItem("cnz_gvl");
					if (t) {
						const e = JSON.parse(t);
						e && e.d && void 0 !== e.d.vendors && (n = e)
					}
				} catch (t) {}
				if (!n || n.u !== t && !this.isLatest) throw new Fe(e.message);
				Date.now() - (n.t || 0) > 6048e5 && console.warn("[Conzent] IAB vendor list served from a cache older than 7 days — the origin is unreachable"), this.populate(n.d)
			}
		}
		getJson() {
			return JSON.parse(JSON.stringify({
				...
				features: this.features,
				specialFeatures: this.specialFeatures,
				// TCF 2.4: also surfaced to __tcfapi('getVendorList').
				standardTexts: this.standardTexts,
				stacks: this.stacks,
				dataCategories: this.dataCategories,
	...
	// ── AC-V2-HELPERS-START ─────────────────────────────────────────────
	// Google Additional Consent, spec v2:
	//   2~<consented ATP ids>~dv.<disclosed but NOT consented ids>
	// Part 5 never repeats an id from part 3 — Google: "Vendors included in
	// Part 3 should not be included in Part 5 to reduce string length."
	//
	// cz._acConsented is the source of truth. The cookie stores ONLY that list,
	// still under the v1 "1~" prefix, and part 5 is derived at runtime: storing
	// the full v2 string puts a reject-all cookie at ~4.2 KB, past the
	// 4096-byte limit, and _cnzEncodeACString cannot help (it compresses only
	// segment [1], and bails entirely when that segment is empty).
	//
	// Ids are sorted ascending for SIZE, not correctness: the delta step in
	// _cnzEncodeACString yields ~900 bytes ascending vs ~1600 unsorted.
	//
	// Defined on cz rather than as bare functions: _tt() and GAC() live in
	// different regions of this hand-concatenated bundle, and cz
	// (= window.conzent) is the only identifier both already dereference.

	cz._acConsented = null;

	cz._acNormalizeIds = function (list) {
		var seen = {}, out = [], arr, i, n;
		if (!list) { return out; }
		arr = Array.isArray(list) ? list : Array.from(list);
		for (i = 0; i < arr.length; i++) {
			n = parseInt(arr[i], 10);        // accepts 42 and "42"; "" -> NaN, dropped
			if (n > 0 && !seen[n]) { seen[n] = 1; out.push(n); }
		}
		return out.sort(function (a, b) { return a - b });
	};

	// The disclosed list, or null when the GVL is not ready. MUST be read after
	// narrowGoogleVendorsTo(), or it will not match what the visitor was shown.
	cz._acDisclosedIds = function () {
		var g = cz._tcModal && cz._tcModal.gvl;
		if (!g) { return null; }
		if (g.googleVendorIds && g.googleVendorIds.size) {
			return cz._acNormalizeIds(Array.from(g.googleVendorIds));
		}
		if (g.googleVendors && Object.keys(g.googleVendors).length) {
			return cz._acNormalizeIds(Object.keys(g.googleVendors));
		}
		return null;
	};

	// disclosed === null -> GVL unavailable, emit an empty dv segment.
	// disclosed === []   -> GVL ready, nothing disclosed. Must not be conflated.
	cz._acBuild = function (consented, disclosed) {
		var c = cz._acNormalizeIds(consented), inC = {}, dv = [], i;
		for (i = 0; i < c.length; i++) { inC[c[i]] = 1; }
		if (disclosed) {
			for (i = 0; i < disclosed.length; i++) {
				if (!inC[disclosed[i]]) { dv.push(disclosed[i]); }
			}
		}
		return "2~" + c.join(".") + "~dv." + dv.join(".");
	};

	// Consented ids out of any AC string, v1 or v2. "1~" and "2~~dv.x" -> [].
	cz._acParse = function (v) {
		if (!v || typeof v !== "string") { return []; }
		var p = v.split("~");
		if (p.length < 2) { return []; }     // no "~" -> not an AC string
		return cz._acNormalizeIds(p[1].split("."));
	};

	// Cookie payload: consented ids only, still under "1~". Byte-identical to
	// what previous builds wrote, so old cookies need no migration, a rollback
	// cannot corrupt anything, and the shared euconsent-v2 slot keeps holding a
	// valid v1 string rather than a malformed v2 one with no dv segment.
	cz._acCookieValue = function () {
		return "1~" + (cz._acConsented || []).join(".");
	};

	// Re-render _addtlConsent from the current consented list.
	cz._acRefresh = function () {
		var disclosed = cz._acDisclosedIds(), known = {}, i;
		if (disclosed) {
			// Drop consent for ATPs no longer disclosed (delisted by Google, or
			// excluded by the publisher's allow-list). Guarded on `disclosed`
			// being non-null — running this against null would wipe everything.
			for (i = 0; i < disclosed.length; i++) { known[disclosed[i]] = 1; }
			cz._acConsented = (cz._acConsented || []).filter(function (id) { return known[id] });
		}
		cz._addtlConsent = cz._acBuild(cz._acConsented, disclosed);
		return cz._addtlConsent;
	};
	// ── AC-V2-HELPERS-END ───────────────────────────────────────────────
	...
	function GAC(e) {
		if (CNZ_config.settings.additional_gcm != 1) {
			// AC disclosure is off for this publisher: nothing consented and
			// nothing disclosed. Never populate part 5 here — Google requires
			// disclosed vendors only where transparency was actually provided.
			// saveTcf() has no additional_gcm guard of its own, so without this
			// a publisher with AC off still gets a real consent string built,
			// cookied and logged the moment a visitor touches the Google tab.
			cz._acConsented = [];
			cz._addtlConsent = "2~~dv.";
			return;
		}
		// Call sites disagree on type: accept-all passes Array.from(a Set of
		// Object.keys) i.e. strings, the custom path passes numbers from the
		// ATP JSON. Normalising here is what keeps the dv set difference sane.
		cz._acConsented = cz._acNormalizeIds(e);
		cz._acRefresh();
	}
	// ── AC-V2-GAC-END ───────────────────────────────────────────────────
	function pnfTabs(){
	...
	function createPnfTabs(){
		var pnHtml = [];
		// TCF 2.4: seed the standard-text slot so the placeholder always has a
		// value. Without this, a falsy cz._pnfTabs leaves the key missing and
		// replaceAll() stringifies undefined into the visible banner.
		pnHtml['features_standard_text'] = '';
		if(cz._pnfTabs){
			cz._pnfTabs.forEach((function(e) {
				if(e.key == 'purposes'){
					...
					pnHtml[e.key+'_count']= e.sublist.length;
					pnHtml[e.key+'_toggle']= toggleHtml;
					// TCF 2.4: IAB's standard explanation for the Features group,
					// rendered on the group header. Sourced from the GVL so it is
					// already translated; stays empty on a pre-2.4 vendor list.
					var _stdTxt = '';
					try {
						_stdTxt = (cz._tcModal && cz._tcModal.gvl && cz._tcModal.gvl.standardTexts && cz._tcModal.gvl.standardTexts.features) || '';
					} catch (_e) { _stdTxt = ''; }
					pnHtml[e.key+'_standard_text'] = _stdTxt ? '<p class="cnz-iab-standard-text">'+_stdTxt+'</p>' : '';
				}
	...
	// default_laws is one of gdpr | ccpa | gdpr_ccpa. Comparing it to the bare
	// string "gdpr" silently excludes gdpr_ccpa, which is a GDPR configuration.
	function cnzGdprApplies(){
		var n = CNZ_config.settings.default_laws;
		return n === 'gdpr' || n === 'gdpr_ccpa';
	}
	function updateTcf(es){
		var cz_action = cookieExists("conzentConsent");
		var tcf_data = (cnzGdprApplies() ? es && cz_action ? "" : cz._cnzStore._tcStringValue : null);
	...
	function saveTcf(e,cl){
		...
		}else if(e =='custom' && cnzGdprApplies()){
		...
		// Store the consented ids only (still "1~"-prefixed). The dv segment is
		// (current ATP list - consented), so persisting it is redundant — and a
		// reject-all v2 string would push this cookie past the 4096-byte limit,
		// where the browser drops it silently and the visitor loses consent.
		Conzent_Cookie.set(CNZ_TCF_COOKIE, "".concat(cz._cnzStore._tcStringValue, ",").concat(cz._cnzEncodeACString(cz._acCookieValue())),CNZ_config.settings.expires,1);
		updateTcf(es);
	}
	function getElm(e,t){
		var r = [],
	...
	// TCF: the spec-mandated cookie name is `euconsent-v2`. Conzent historically
	// wrote plain `euconsent`, so reads prefer the correct name and fall back to
	// the legacy one, republishing it in place. Visitors who already consented
	// are migrated silently on their next page view — never re-prompted.
	var CNZ_TCF_COOKIE = "euconsent-v2",
		CNZ_TCF_COOKIE_LEGACY = "euconsent";

	function cnzReadTcfCookie(){
		var v = Conzent_Cookie.read(CNZ_TCF_COOKIE);
		if(v){ return v; }
		var legacy = Conzent_Cookie.read(CNZ_TCF_COOKIE_LEGACY);
		if(legacy){
			Conzent_Cookie.set(CNZ_TCF_COOKIE, legacy, CNZ_config.settings.expires, 1);
			Conzent_Cookie.erase(CNZ_TCF_COOKIE_LEGACY, 1);
			return legacy;
		}
		return "";
	}

	function cnzEraseTcfCookie(){
		Conzent_Cookie.erase(CNZ_TCF_COOKIE, 1);
		Conzent_Cookie.erase(CNZ_TCF_COOKIE_LEGACY, 1);
	}
	...
	function loadIabtcf(){
		...
		if(CNZ_config.settings.iab_support == 1){
			_t1();
			var l_n = (cnzReadTcfCookie() || "").split(",");
			...
			if(CNZ_config.settings.additional_gcm == 1){
				cz._acConsented = cz._acParse(o);
				// The GVL is not loaded yet, so part 5 cannot be derived. Serve a
				// well-formed v2 string with an empty dv until readyPromise
				// resolves — under-disclosing is the conservative direction and
				// can never over-claim consent. With no stored record at all,
				// keep serving "" rather than asserting a disclosure that has
				// not happened yet.
				cz._addtlConsent = o ? cz._acBuild(cz._acConsented, null) : "";
			}
			else{
				cz._acConsented = [];
				cz._addtlConsent = '2~~dv.';
			...
			at = pt(new bn([CNCMPID],[CNCMPVERSION], !0, {
				getTCData: (t, e, n) => {
					"boolean" != typeof e && (e.addtlConsent = cz._addtlConsent, e.enableAdvertiserConsentMode = 1 == (CNZ_config.settings && CNZ_config.settings.advertiser_consent_mode)), t(e, n)
				}
			}));
			cz._cmpAPI = at;	
			_t1();	
			// `st` is a Preact internal, so st.vendorListVersion was always
			// undefined and this only ever loaded "latest" by accident. Say so
			// explicitly — passing a real version here would fetch
			// archives/vendor-list-v[N].json instead of the current list.
			const u = new ln(new pn());
			// TCF: GVL purpose/feature/vendor text must be disclosed in the
			// visitor's language. Assigning gvl.lang_ directly (as this used to)
			// only relabelled the instance — it never fetched
			// purposes-[LANG].json, so every non-English visitor was shown
			// English disclosures inside translated banner chrome. Go through
			// changeLanguage() instead, and fall back to English if IAB
			// publishes no file for that language.
			var _lang = CNZ_config.currentLang.toUpperCase().replace("_","-") || "EN";
			u.cmpId = [CNCMPID], u.cmpVersion = [CNCMPVERSION], u.consentScreen = 1, u.consentLanguage = _lang, u.isServiceSpecific = !0, u.publisherCountryCode = CNZ_config.publisherCountry.toUpperCase()||"AA", u.gvl.readyPromise.then((() => u.gvl.changeLanguage(_lang).catch((() => {})))).then((async () => {
				// Lazy translations must be hydrated before the banner renders
				if (window._cnzI18nReady) await window._cnzI18nReady;
				var new_tcModel = u;
				new_tcModel.purposeLegitimateInterests.set([2, 7, 8, 9, 10, 11]);
				new_tcModel.setAllVendorLegitimateInterests();
				r ? ((t = dn.decode(r)).policyVersion_ < u.tcfPolicyVersion ? (new_tcModel, cnzEraseTcfCookie()) : (t.gvl = u.gvl, cz._cnzStore._tcStringValue = r), o && (cz._addtlConsent = o)) : t = u, cz._tcModal = t;
				if(CNZ_config.settings.additional_gcm == 1){
				...
				// The GVL is loaded and publisher narrowing has been applied, so
				// part 5 is now derivable AND matches the list the visitor is
				// actually shown. This is also what upgrades a stored v1 consent
				// list to a served v2 string. It must run after the narrowing —
				// narrowGoogleVendorsTo() rebuilds googleVendorIds.
				if(CNZ_config.settings.additional_gcm == 1 && (o || (cz._acConsented && cz._acConsented.length))){
					cz._acRefresh();
				}
				// pnfTabs() and the vendor count run after the narrowing too, so
				// the rendered tab and the {vendor_count} figure describe the same
				// vendor set the TC string and the AC string do.
				pnfTabs();
				...
				if (window._cnzI18nReady) await window._cnzI18nReady;
				// Degraded mode: no vendor list from network or cache (cold-start
				// visitor during an origin outage). Without this catch the chain
				// died silently: no banner, and vendors hung on cmpStatus 'stub'
				// forever. Tear the CMP down per spec — disable() sets cmpStatus
				// 'error', purging the queue answers every waiting vendor call
				// with the error TCData so they fall back to non-personalised —
				// then boot the plain banner: consent collection and blocking
				// keep working through the outage.
				try { at.disable(), at.callResponder.purgeQueuedCalls() } catch (t) {}
				CNZ_config.settings.iab_support = 0;
				CNZ_config._tcfDegraded = !0;
				try { showConsoleError("IAB vendor list unavailable (" + (e && e.message ? e.message : e) + ") — banner running in degraded non-TCF mode") } catch (t) {}
				Conzent.init();
				var _cn = _QS('#Conzent');
				if (_cn) { _cn.innerHTML = _cn.innerHTML.replace(/\{vendor_count\}/g, "").replace(/\{\{count\}\}/g, "") }
			}));
		}
	}
	function loadIabtcf_preview(){
		...
		if(CNZ_config.settings.iab_support == 1){
			...
			// archives/vendor-list-v[N].json instead of the current list.
			const u = new ln(new pn());
			// Preview path — mirror the live language handling above so the
			// admin preview shows what visitors actually get.
			var _lang = CNZ_config.currentLang.toUpperCase().replace("_","-") || "EN";
			u.cmpId = [CNCMPID], u.cmpVersion = [CNCMPVERSION], u.consentScreen = 1, u.consentLanguage = _lang, u.isServiceSpecific = !0, u.publisherCountryCode = CNZ_config.publisherCountry.toUpperCase()||"AA", u.gvl.readyPromise.then((() => u.gvl.changeLanguage(_lang).catch((() => {})))).then((async () => {
				// Lazy translations must be hydrated before the banner renders
				if (window._cnzI18nReady) await window._cnzI18nReady;
				...
				// The preview never assigned _addtlConsent here at all, so without
				// this the Google tab renders with every toggle unchecked and the
				// admin preview disagrees with production. No narrowing on this
				// path, so the refresh can run immediately.
				if(CNZ_config.settings.additional_gcm == 1){
					cz._acRefresh();
				}
				pnfTabs();
				...
				if (window._cnzI18nReady) await window._cnzI18nReady;
				// Mirror the live degraded path so the admin preview shows what
				// visitors actually get when the vendor list is unreachable.
				try { at.disable(), at.callResponder.purgeQueuedCalls() } catch (t) {}
				CNZ_config.settings.iab_support = 0;
				CNZ_config._tcfDegraded = !0;
				try { showConsoleError("IAB vendor list unavailable (" + (e && e.message ? e.message : e) + ") — banner running in degraded non-TCF mode") } catch (t) {}
