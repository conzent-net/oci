...
/**
 * GET /policies/cookie — 3-step cookie policy wizard.
 *
 * Supports ?template_id=X to edit a template's fields using the same wizard.
 */
final class CookiePolicyWizardHandler implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($templateId > 0) {
            try {
                $policy = $this->policyService->getTemplateForEditing($templateId, 'cookie', (int) $user['id']);
            } catch (\RuntimeException) {
                return ApiResponse::redirect('/policies');
            }
        } else {
            $policy = $this->policyService->getOrCreateCookiePolicy($siteId, $languageId);
        }

        $response = ApiResponse::html($this->twig->render('pages/policies/cookie_wizard.html.twig', [
            'title' => $templateId > 0 ? 'Edit Cookie Template' : 'Cookie Policy',
            'user' => $user,
            'siteId' => $siteId,
            'sites' => $sites,
