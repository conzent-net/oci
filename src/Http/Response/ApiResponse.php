...
final class ApiResponse
{
    ...
    /**
     * @param array<string, mixed> $errors Field name → message map for inline display.
     * @param array<string, mixed> $extra  Additional keys merged into the error object —
     *                                     e.g. a fresh csrf_token so single-use tokens
     *                                     survive a failed attempt and the user can retry.
     */
    public static function error(string $message, int $status = 400, array $errors = [], array $extra = []): ResponseInterface
    {
        $body = [
            'success' => false,
            'error' => [
