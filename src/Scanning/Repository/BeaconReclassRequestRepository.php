...
final class BeaconReclassRequestRepository implements BeaconReclassRequestRepositoryInterface
{
    ...
    {
        $this->db->insert('oci_beacon_reclass_requests', [
            'site_id' => $data['site_id'],
            'user_id' => $data['user_id'],
            'beacon_domain' => $data['beacon_domain'],
    ...
    public function findById(int $id): ?array
    {
        $row = $this->db->fetchAssociative(
            'SELECT r.*, s.domain AS site_domain,
             ...
             FROM oci_beacon_reclass_requests r
    ...
    {
        $id = $this->db->fetchOne(
            "SELECT id FROM oci_beacon_reclass_requests
             WHERE site_id = :siteId AND beacon_domain = :domain AND status = 'pending'
             LIMIT 1",
            ['siteId' => $siteId, 'domain' => $beaconDomain],
        );
    ...
    public function countPending(): int
    {
        return (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM oci_beacon_reclass_requests WHERE status = 'pending'",
        );
    }
    ...
    public function getPendingDomainsForSite(int $siteId): array
    {
        $rows = $this->db->fetchFirstColumn(
            "SELECT beacon_domain FROM oci_beacon_reclass_requests
