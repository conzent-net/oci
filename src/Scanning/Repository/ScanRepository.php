...
final class ScanRepository implements ScanRepositoryInterface
{
    ...
    {
        // A scan is due once its full scheduled moment (date + time) has passed
        // — the same rule for one-time and monthly scans.
        //
        // The previous monthly check matched on DAY(schedule_date) = DAY(today)
        // alone, ignoring month/year. Because a fired monthly scan clones itself
        // forward by exactly one month (preserving the day-of-month), that clone
        // re-matched "today" on every scheduler cycle and spawned a fresh scan
        // every 5 minutes for the whole day. Comparing the concrete next-run
        // datetime means a monthly scan is due only when its month has arrived,
        // and then fires just once.
        $now = (new \DateTimeImmutable())->format('Y-m-d H:i:s');
        ...
        return $this->db->fetchAllAssociative(
            "SELECT s.*, st.domain
             ...
             AND s.frequency IN ('once', 'monthly')
             AND s.schedule_date IS NOT NULL
             AND TIMESTAMP(s.schedule_date, COALESCE(s.schedule_time, '00:00:00')) <= :now
    ...
    {
        return $this->db->fetchAllAssociative(
            "SELECT * FROM oci_scan_cookies WHERE scan_id = :scanId AND cookie_name <> '' ORDER BY cookie_name ASC",
            ['scanId' => $scanId],
        );
    }
    ...
    {
        return (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM oci_scan_cookies WHERE scan_id = :scanId AND cookie_name <> ''",
            ['scanId' => $scanId],
        );
    }
    ...
    public function getScanCookieBreakdown(int $scanId): array
    {
        return $this->db->fetchAllAssociative(
            "SELECT COALESCE(category_slug, 'unclassified') AS category_slug, COUNT(*) AS total
             ...
             WHERE scan_id = :scanId AND cookie_name <> ''
    ...
    public function getBeaconsByScan(int $scanId): array
    {
        return $this->db->fetchAllAssociative(
            "SELECT b.*, bs.found_at
             ...
             WHERE bs.scan_id = :scanId AND b.beacon_url <> ''
    ...
    {
        return (int) $this->db->fetchOne(
            "SELECT COUNT(*) FROM oci_beacon_scans bs
             INNER JOIN oci_beacons b ON b.id = bs.beacon_id
             WHERE bs.scan_id = :scanId AND b.beacon_url <> ''",
            ['scanId' => $scanId],
        );
    }
