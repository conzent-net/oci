 ...
 *
 * Caveat: the cache keys on numeric oci_cookies_global ids, which are only
 * stable for the database the cache was generated against. On a fresh install
 * synced from the Open Cookie Database, ids that don't exist are skipped and
 * ids that shifted may attach a translation to a neighbouring cookie. The
 * durable fix is re-keying the cache by the stable source UUID
 * (oci_cookies_global.cookie_id) and regenerating it.
 */
final class CookieTranslationSeeder
{
    // gemini-2.0-flash-001 was retired from OpenRouter mid-2026 ("No endpoints found")
    ...
    public function insertIntoDatabase(array $translations, bool $dryRun = false): array
    {
        ...
        foreach ($translations as $langCode => $cookies) {
            foreach ($cookies as $cookieId => $description) {
                ...
                // Joined on oci_cookies_global so a cached id that does not
                // exist in this install no-ops instead of failing the FK and
                // aborting the whole run. Cache ids come from the database the
                // cache was generated against; a fresh install's ids can be a
                // subset (or shifted — see the class docblock caveat).
                $affected = $this->db->executeStatement(<<<'SQL'
                    INSERT INTO oci_cookies_global_translations (cookie_id, language_id, description)
                    SELECT g.id, l.id, :description
                    FROM oci_cookies_global g
                    CROSS JOIN oci_languages l
                    WHERE g.id = :cookieId AND l.lang_code = :langCode
