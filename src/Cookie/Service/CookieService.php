...
final class CookieService
{
    ...
    public function getCookiesGroupedByCategory(int $siteId, ?string $search = null): array
    {
        ...
        // Drop glitch cookies with no name — assume they are scan artefacts.
        $allCookies = array_values(array_filter(
            $allCookies,
            static fn(array $c): bool => trim((string) ($c['cookie_name'] ?? '')) !== '',
        ));
        ...
        // Apply per-site user classifications (highest priority) so a
        // user-classified cookie appears under its chosen category.
        $overrides = $this->cookieRepo->getSiteClassificationOverrides($siteId);
        if ($overrides !== []) {
            foreach ($allCookies as &$cookie) {
                $key = mb_strtolower($cookie['cookie_name'] ?? '');
                if (isset($overrides[$key])) {
                    $cookie['category_slug'] = $overrides[$key]['category_slug'];
                    $cookie['category_name'] = $overrides[$key]['category_name'];
                }
            }
            unset($cookie);
        }
    ...
    /**
     * Classify an unclassified cookie into a category (per-site, immediate).
     *
     * Cookies already classified by Conzent's global database are locked and
     * must go through the reclassification-request flow instead.
     *
     * @return array{success: bool, error?: string}
     */
    public function classifyCookie(int $userId, int $siteId, string $cookieName, ?string $cookieDomain, int $categoryId): array
    {
        $site = $this->siteRepo->findById($siteId);
        if ($site === null || (int) $site['user_id'] !== $userId) {
            return ['success' => false, 'error' => 'Site not found'];
        ...
        // Guard: a cookie classified by Conzent's global DB cannot be
        // reclassified directly — it must go through a request.
        $global = $this->cookieRepo->matchGlobal($cookieName, $cookieDomain);
        if ($global !== null && !empty($global['category_id'])) {
            return [
                'success' => false,
                'error' => 'This cookie is classified by Conzent. Submit a reclassification request instead.',
            ];
        }

        // Validate the target category exists and isn't "unclassified".
        ...
        $this->cookieRepo->upsertSiteClassification($siteId, $cookieName, $cookieDomain, $categoryId);
    ...
    public function getCategoryList(int $siteId): array
    {
        ...
        $default = $this->languageRepo->getDefaultLanguage($siteId);

        return [
            'categories' => $categories,
            'globalCategories' => $this->categoryRepo->getAllGlobalCategories(),
            'languages' => $this->languageRepo->getSiteLanguages($siteId),
            'defaultLanguageId' => (int) ($default['lang_id'] ?? 1),
            'categoryTranslations' => $this->categoryRepo->getSiteCategoryTranslations($siteId),
        ];
    }
    ...
    public function updateSiteCategory(int $userId, int $siteCategoryId, array $data): array
    {
        ...
        // Resolve and validate the translation language before writing anything,
        // so an invalid language rejects the whole update instead of half of it.
        $languageId = 0;
        if (isset($data['name'])) {
            $siteId = (int) $cat['site_id'];
            $languageId = (int) ($data['language_id'] ?? 0);

            if ($languageId > 0) {
                $siteLanguageIds = array_map(
                    static fn (array $lang): int => (int) $lang['id'],
                    $this->languageRepo->getSiteLanguages($siteId),
                );
                if (!\in_array($languageId, $siteLanguageIds, true)) {
                    return ['success' => false, 'error' => 'Language is not enabled for this site'];
                }
            } else {
                $default = $this->languageRepo->getDefaultLanguage($siteId);
                $languageId = (int) ($default['lang_id'] ?? 1);
            }
        }
