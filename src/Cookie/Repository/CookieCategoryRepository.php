...
final class CookieCategoryRepository implements CookieCategoryRepositoryInterface
{
    ...
    public function getSiteCategoryTranslations(int $siteId): array
    {
        $rows = $this->db->fetchAllAssociative(
            'SELECT scct.site_cookie_category_id, scct.language_id, scct.name, scct.description
             FROM oci_site_cookie_category_translations scct
             INNER JOIN oci_site_cookie_categories scc ON scc.id = scct.site_cookie_category_id
             WHERE scc.site_id = :siteId',
            ['siteId' => $siteId],
        );
        ...
        $map = [];
        foreach ($rows as $row) {
            $map[(int) $row['site_cookie_category_id']][(int) $row['language_id']] = [
    ...
    public function findCategory(int $id, int $languageId = 1): ?array
    {
        $row = $this->db->fetchAssociative(
            'SELECT c.id, c.slug, c.type, c.sort_order, c.default_consent, ct.name, ct.description
             FROM oci_cookie_categories c
