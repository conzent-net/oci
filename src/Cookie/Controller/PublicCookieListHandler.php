...
{
    /** @var array<string, string> lowercase cookie name => override slug */
    private array $classificationOverrides = [];
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        // Per-site user classifications take priority over scan/global categories.
        $this->classificationOverrides = $this->loadClassificationOverrides($siteId);
    ...
    /**
     * Load per-site user classifications (from_scan = 0) as name => slug.
     *
     * @return array<string, string>
     */
    private function loadClassificationOverrides(int $siteId): array
    {
        $rows = $this->db->fetchAllAssociative(
            'SELECT sc.cookie_name, cc.slug
             FROM oci_site_cookies sc
        ...
        $map = [];
        foreach ($rows as $row) {
            $map[strtolower($row['cookie_name'])] = $row['slug'];
        }
    ...
    private function groupByCategory(array $rows, string $slugField, string $durationField): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($name === null || trim((string) $name) === '') {
                continue; // glitch cookie with no name
            }
            if (isset($seen[$name])) {
                continue;
            }
            ...
            // A user classification for this cookie overrides everything else.
            $override = $this->classificationOverrides[strtolower($name)] ?? null;
            if ($override !== null) {
                $slug = $override;
            }
            ...
            $cookies[$slug][] = [
                ...
                'description' => array_merge(
                    ['en' => $row['global_description'] ?? ''],
                    $row['global_translations'] ?? [],
                ),
    ...
    private function enrichFromGlobalDatabase(array $cookies): array
    {
        ...
        $exactMatches = $this->db->fetchAllAssociative(
            "SELECT cg.id AS global_id, cookie_name, cg.description, expiry_duration, platform,
                    cc.slug AS category_slug
        ...
        foreach ($cookies as &$cookie) {
            ...
            if (isset($exactMap[$nameLower])) {
                ...
                $cookie['global_id'] = (int) $match['global_id'];
                $matchedGlobalIds[] = (int) $match['global_id'];
            ...
            foreach ($wildcardPatterns as $pattern) {
                ...
                if (preg_match($regex, $cookie['cookie_name'])) {
                    ...
                    $cookie['global_id'] = (int) $pattern['global_id'];
                    $matchedGlobalIds[] = (int) $pattern['global_id'];
                    if (empty($cookie['expiry_duration'])) {
        ...
        // Batch-load translations for all matched global cookies
        $translationMap = $this->loadGlobalTranslations($matchedGlobalIds);

        foreach ($cookies as &$cookie) {
            $gid = $cookie['global_id'] ?? null;
            $cookie['global_translations'] = $gid !== null ? ($translationMap[$gid] ?? []) : [];
        }
        unset($cookie);
    ...
    /**
     * Load all translations for a set of global cookie IDs.
     *
     * @param int[] $globalIds
     * @return array<int, array<string, string>> [global_id => [lang_code => description]]
     */
    private function loadGlobalTranslations(array $globalIds): array
    {
        if (empty($globalIds)) {
            return [];
        }
        ...
        $rows = $this->db->fetchAllAssociative(
            "SELECT cgt.cookie_id, l.lang_code, cgt.description
             FROM oci_cookies_global_translations cgt
             INNER JOIN oci_languages l ON cgt.language_id = l.id
             WHERE cgt.cookie_id IN ({$placeholders})
               AND cgt.description IS NOT NULL AND cgt.description != ''",
            array_values(array_map('intval', $globalIds)),
        );

        $map = [];
        foreach ($rows as $row) {
            $map[(int) $row['cookie_id']][$row['lang_code']] = $row['description'];
        }
