...
final class EndpointrClient
{
     ...
     *
     * The provider is requested EXPLICITLY (default reoon, override via
     * ENDPOINTR_VERIFY_PROVIDER). Without it, endpointr walks the customer's
     * admin-side "verification priority" list — a remote config dependency
     * that, when empty, 404s every call and silently disabled the signup
     * gate (which fails open by design). Explicit provider + a stored
     * credential works regardless of that list.
     *
    ...
    public function verifyEmail(string $email): array
    {
        ...
        $provider = trim((string) ($_ENV['ENDPOINTR_VERIFY_PROVIDER'] ?? 'reoon'));
        if ($provider !== '') {
            $body['provider'] = $provider;
        }

        // NB: the route is /v1/verifications/verify — the bare /v1/verifications
        // does not exist and 404s ("Route not found"). This client shipped with
        // the wrong path, which silently disabled BOTH consumers: the nightly
        // batch and the signup gate (each degrades gracefully on failure, so
        // nothing ever surfaced until the bot wave of 2026-08-27).
