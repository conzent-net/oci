    ...
    public function attemptGoogle(array $profile, string $ip, string $userAgent): array
    {
        ...
        if ($user === false) {
            ...
            $this->db->insert('oci_users', [
                'username' => $username,
                'email' => $email,
                'email_verified' => 'VALID', // OAuth provider has already verified the address
    ...
    public function createSession(array $user, string $ip, string $userAgent, bool $remember = false): string
    {
        ...
        if ($remember) {
            ...
            setcookie('oci_remember', $cookieValue, [
                ...
                'httponly' => true,
                'samesite' => 'Lax',
                // Derived from APP_URL, not APP_ENV: installs set APP_ENV to
                // "production" (not "prod"), which silently left this 30-day
                // cookie without the Secure flag on every HTTPS deployment.
                'secure' => str_starts_with((string) ($_ENV['APP_URL'] ?? ''), 'https://'),
            ]);
        }
