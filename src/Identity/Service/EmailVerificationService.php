...
final class EmailVerificationService
{
    ...
    public function startSignup(array $formData): array
    {
        ...
        // Deep verification (Reoon via endpointr), deliberately LAST and in
        // ADDITION to the free checks above: syntax/disposable/MX already
        // rejected the garbage for free, so only plausible addresses spend a
        // metered credit — and the per-IP rate limit caps how fast a bot can
        // burn them. This catches what no free check can: a deliverable-looking
        // mailbox that does not exist (accept-then-bounce at big providers).
        // Fails open — a verifier outage must never block real signups.
        try {
            $verdict = $this->endpointr->verifyEmail($formData['email']);
            $status = strtolower((string) ($verdict['status'] ?? ''));
            $raw = \is_array($verdict['raw'] ?? null) ? $verdict['raw'] : [];

            // Reject on a hard-invalid status, and additionally whenever Reoon
            // says the address is not safe to send to — EXCEPT the two classes
            // where "unsafe" really means "unknowable": catch-all domains and
            // role accounts, both routine for legitimate small-business
            // signups (info@firma.dk). Blocking those would cost customers.
            $unsafe = ($raw['is_safe_to_send'] ?? null) === false
                && ($raw['is_role_account'] ?? false) !== true
                && ($raw['is_catch_all'] ?? false) !== true;

            if ($this->mapStatus($status) === self::STATUS_INVALID || $unsafe) {
                $this->logger->info('Signup rejected by deep email verification', [
                    'email' => $formData['email'],
                    'status' => $status,
                    'is_safe_to_send' => $raw['is_safe_to_send'] ?? null,
                ]);

                return ['success' => false, 'errors' => ['email' => EmailValidator::ERROR_MESSAGE]];
            }
        } catch (\Throwable $e) {
            // ERROR, not warning: this gate FAILING OPEN is how bot addresses
            // reach SES. A persistent version of this line means the endpointr
            // credential or verification priority is broken — run
            // `php bin/oci email:verify <address>` to see the exact failure.
            $this->logger->error('Deep email verification unavailable — allowing signup', [
                'email' => $formData['email'],
                'exception' => $e::class,
                'error' => $e->getMessage(),
            ]);
        ...
        if (!$this->sendCodeEmail($formData['email'], $formData['first_name'], $code)) {
            // Don't leave a stash the user can never complete — a dead stash
            // blocks re-registration until the code TTL expires.
            unset($_SESSION[self::SESSION_KEY]);

            return ['success' => false, 'errors' => [
                'email' => 'We could not send the verification email. Please try again in a few minutes.',
            ]];
        }
    ...
    public function resendCode(): array
    {
        ...
        if (!$sent) {
            // Cooldown stays armed on purpose: if SES is refusing sends,
            // an immediate retry would only hammer it.
            return ['success' => false, 'error' => 'We could not send the verification email. Please try again in a few minutes.'];
        }
    ...
    private function sendCodeEmail(string $email, string $firstName, string $code): bool
    {
        ...
        $sent = $this->mailer->send($email, 'Your Conzent verification code', $html);
        if (!$sent) {
            // MailerService has already logged the SMTP detail; this line ties
            // the failure to the signup flow so it is findable from here.
            $this->logger->error('Verification code email failed to send', ['email' => $email]);
        }
    ...
    private function validateForm(array $data): array
    {
        ...
        if ($email === '') {
            ...
            // Syntax + disposable-domain + MX checks. Every address that gets
            // past this point costs a real SES send, and a bounce is charged
            // against the domain's sending reputation — reject here, for free.
            $emailError = $this->emailValidator->validate($email);
            if ($emailError !== null) {
