...
final class SiteCreationService
{
    ...
    private function initiateFirstScan(int $siteId, string $domain, int $userId): void
    {
        ...
        // Queue the first scan for real. The old behavior wrote a scan row
        // with status 'initiated' that no worker ever picked up, so new sites
        // silently never got their promised automatic scan. queueFirstScan
        // never throws; on failure the dashboard scan step offers a manual
        // start.
        if ($this->scanService !== null) {
            $this->scanService->queueFirstScan($siteId);
        }
        ...
        if ($this->hasFeature('monthly_scheduled_scan', $planFeatures)) {
            ...
            $this->scanRepo->createScan([
                ...
                'total_pages' => $this->resolveScanLimit($userId),
            ]);
        }
    }
