...
final class GtmWizardService
{
     ...
     *
     * version + signature MUST be a pair the gallery has actually issued:
     * the signature is minted by Google when the gallery ingests a release
     * and is not derivable locally. To refresh after a template release
     * reaches the gallery, read the pair from any container that installed
     * the template from the gallery:
     *   GET tagmanager/v2/accounts/{a}/containers/{c}/workspaces/{w}/templates
     * (the cmptest.conzent.net container carries a gallery install for
     * exactly this purpose). The template CODE the wizard installs comes
     * from public/assets/gtm/templates/conzent.tpl regardless - keep that
     * file current with github.com/conzent-net/conzent_cmp.
     */
    private const CONZENT_GALLERY_REF = [
        'host' => 'github.com',
        'owner' => 'conzent-net',
        'repository' => 'conzent_cmp',
        'version' => '23a3fb38edee6329be457cd5e0afcecda4eeee55',
        'signature' => 'd3d27b78754371eaf0b174a2aa5dec2e6e491b52a5550abb4971398cddbff6e6',
    ];
    ...
    public function createFoundation(string $token, string $wsPath, string $websiteKey): array
    {
            ...
            'customEventFilter' => [
                [
                    ...
                    'parameter' => [
                        ...
                        ['key' => 'arg1', 'type' => 'template', 'value' => 'cookie_consent_update'],
                    ],
                ],
            ],
        ];
        ...
        $tag = [
            ...
            'parameter' => [
                ...
                // Explicit, not left to the template default: this wizard serves
                // the GetConzent platform, and an explicit stored value survives
                // any future default change in the template.
                ['key' => 'platform', 'type' => 'template', 'value' => 'getconzent'],
