...
final class SiteRepository implements SiteRepositoryInterface
{
    ...
    public function getAssociatedDomains(int $siteId): array
    {
        return $this->db->fetchAllAssociative(
            'SELECT id, domain, privacy_policy_url, created_at
             FROM oci_associated_sites
    ...
    public function associatedDomainExists(int $siteId, string $domain): bool
    {
        $result = $this->db->fetchOne(
            'SELECT 1 FROM oci_associated_sites WHERE site_id = :siteId AND domain = :domain',
            ['siteId' => $siteId, 'domain' => $domain],
        );

        return $result !== false;
    }

    public function addAssociatedDomain(int $siteId, string $domain, ?string $privacyPolicyUrl): int
    {
    ...
    public function removeAssociatedDomain(int $id, int $siteId): bool
    {
        return $this->db->delete('oci_associated_sites', ['id' => $id, 'site_id' => $siteId]) > 0;
    }
}
