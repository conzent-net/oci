...
final class AssociatedDomainAddHandler implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($domain === $siteDomain) {
            // "www." was stripped from the input above, so a www entry lands
            // here as a duplicate of the primary domain. Saying only "already
            // the primary domain" reads as a mistake to someone who typed a
            // visibly different host, so explain why it is already covered:
            // consent is scoped to the apex, which every subdomain inherits.
            $typed = strtolower(trim($rawDomain));
            $error = str_starts_with($typed, 'www.')
                ? sprintf(
                    '%s is already covered by your primary domain (%s). Subdomains, including www, share consent automatically, so you do not need to add them here.',
                    $typed,
                    $siteDomain,
                )
                : 'That is already this site\'s primary domain';

            return ApiResponse::json(['success' => false, 'error' => $error], 422);
        }
