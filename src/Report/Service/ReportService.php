...
final class ReportService
{
    ...
    public function generate(int $siteId, int $userId, string $reportType, string $start, string $end): int
    {
        ...
        if (\in_array($reportType, ['scan', 'full'], true)) {
            ...
            // Register change log for the period — decoded here so the mail
            // template renders plain values, not JSON strings.
            try {
                $rows = $this->registerRepo->getChangesSince($siteId, $start . ' 00:00:00');
                $data['register_changes'] = array_map(static function (array $row): array {
    ...
    /**
     * Generate and email a scan report after a scheduled scan completes.
     *
     * The report covers the last 30 days of cookie observations, so the mail
     * shows the fresh scan result in the context of recent pre-consent
     * violations. Stored like any other report, so it also appears on the
     * Reports page. Delivery goes to the scan-report schedule's address when
     * one exists, otherwise to the site owner.
     */
    public function sendScheduledScanReport(int $siteId): void
    {
        $site = $this->siteRepo->findById($siteId);
        if ($site === null) {
            return;
        ...
        $reportId = $this->generate($siteId, $userId, 'scan', $start, $end);
        ...
        if ($to === null || $to === '') {
            $this->logger->warning('Scheduled scan report generated but no recipient found', [
                'site_id' => $siteId,
                'report_id' => $reportId,
            ]);
