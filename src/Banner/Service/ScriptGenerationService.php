...
{
    /**
     * Conzent Cloud's own registered IAB CMP ID. Used only when CMP_ID is
     * unset; self-hosters must register with IAB Europe and set their own,
     * because this value is encoded into every TC string the install issues.
     */
    private const DEFAULT_CMP_ID      = 446;
    private const DEFAULT_CMP_VERSION = 3;

    /** IAB reserves 0 and 1, so a real CMP ID is always >= 2. */
    private const MIN_CMP_ID = 2;
    ...
    public function __construct(
        ...
        $this->cmpId = $this->resolveCmpId();
        $this->cmpVersion = $this->resolveCmpVersion();
    }

    /**
     * The CMP ID baked into every TC string this install issues.
     *
     * Was hardcoded to Conzent Cloud's ID, which meant self-hosters silently
     * signed their TC strings as Conzent. Now honours CMP_ID, falling back to
     * the Conzent Cloud ID only when nothing is configured.
     *
     * Registration is not verified here — an unregistered ID is logged loudly
     * but still used, because refusing to generate would take a running
     * install offline over a registry lookup.
     */
    private function resolveCmpId(): int
    {
        $configured = trim((string) ($_ENV['CMP_ID'] ?? ''));

        if ($configured === '' || $configured === '0') {
            return self::DEFAULT_CMP_ID;
        }

        if (!ctype_digit($configured) || (int) $configured < self::MIN_CMP_ID) {
            // [CNCMPID] is substituted into bare JS expression positions and the
            // CmpApi constructor throws on a non-integer, so a bad value here
            // would emit a broken script for every TCF site.
            $this->logger->error(
                "CMP_ID \"{$configured}\" is not a valid IAB CMP ID (expected an integer >= "
                . self::MIN_CMP_ID . '). Falling back to ' . self::DEFAULT_CMP_ID . '.',
            );

            return self::DEFAULT_CMP_ID;
        }

        $cmpId = (int) $configured;

        if ($cmpId !== self::DEFAULT_CMP_ID) {
            $this->logger->info("Generating TCF scripts with CMP ID {$cmpId}.");
        }

        return $cmpId;
    }

    private function resolveCmpVersion(): int
    {
        $configured = trim((string) ($_ENV['CMP_VERSION'] ?? ''));

        if ($configured === '' || !ctype_digit($configured) || (int) $configured < 1) {
            return self::DEFAULT_CMP_VERSION;
        }
    ...
    public function generate(int $siteId): bool
    {
        try {
            ...
            // The consent cookie is scoped to this host, so a "www." prefix must
            // never survive. A cookie on ".www.example.com" is rejected outright
            // by the browser when the visitor is on the apex (www is a CHILD of
            // the apex, not a parent), which silently breaks consent persistence
            // and re-shows the banner on every page load. Scoping to the apex is
            // correct in both directions, because ".example.com" is sent to
            // www.example.com as well. Stored data is left untouched; this is
            // normalised at generation so existing www-registered sites are
            // fixed by a regenerate alone.
            $siteDomainHostname = self::stripWwwPrefix((string) $siteDomainHostname);
            ...
            // Keyed by the full normalised host, NOT by getDomainOnly(): that
            // helper keeps only the last two labels, so an associated domain on
            // a subdomain (test.example.com) collapsed onto the primary's key
            // and overwrote its policy URL, and example.co.uk collapsed to
            // co.uk. The runtime looks these up by hostname, so full hosts are
            // also the only keys that can ever match.
            $policyList = [$siteDomainHostname => $defaultPrivacyPolicyUrl];
            $associatedDomains = $this->loadAssociatedDomains($siteId, $userId);
            foreach ($associatedDomains as $ad) {
                $associatedHost = self::stripWwwPrefix((string) preg_replace('#:\d+$#', '', (string) $ad['domain']));
                $allowedDomains[] = $associatedHost;

                // The per-domain privacy policy is optional. When it is blank,
                // inherit the site's own policy rather than storing an empty
                // string, so an associated domain always has a working link.
                $associatedPolicy = $this->normalizeUrl($ad['privacy_policy'] ?? '');
                $policyList[$associatedHost] = $associatedPolicy !== ''
                    ? $associatedPolicy
            ...
            foreach ($bannerList as $bannerinfo) {
                ...
                // ── Bundle slimming: translations leave the bundle ──
                // shortCodes and the per-category texts carry every site
                // language (up to 86) while a visitor uses exactly one — that
                // was ~50-60% of the bundle. Keep only the main language
                // inline; every other language becomes a lazy side file the
                // runtime fetches when the page language differs (and falls
                // back to the main language when it can't).
                $split = $this->splitTranslationsByLanguage($configArray, $defaultMainCode);
                $configArray = $split['config'];
                $configArray['_i18n'] = $this->writeI18nFiles($websiteKey, $cookieLaws, $split['i18n']);
            ...
            if ($isIab === 1) {
                ...
                $iab_replace_tags = "if(CNZ_config.settings.iab_support == 1){\n"
                    ...
                    // TCF 2.4 Features standard text. The `|| ''` guards against a
                    // site still serving a cached pre-2.4 bundle, where the key is
                    // absent and replaceAll would print the word "undefined".
                    . "\t\t\t\t\t\t\t\t\tcookieNotice = cookieNotice.replaceAll('[IAB_FEATURES_STANDARD_TEXT]',iab_tabs['features_standard_text'] || '');\n"
            ...
            // Core replacements
            $mainScript = str_replace('[CNCMPID]', (string) $this->cmpId, $mainScript);
            $mainScript = str_replace('[CNCMPVERSION]', (string) $this->cmpVersion, $mainScript);
            ...
            if ($IAB2_STUB !== '' || $IAB2_SCRIPT !== '') {
                ...
                $minified = str_replace('[CNCMPID]', (string) $this->cmpId, $minified);
                $minified = str_replace('[CNCMPVERSION]', (string) $this->cmpVersion, $minified);
                $minified = str_replace('[WEB_PATH]', $this->webRoot, $minified);
                file_put_contents($scriptPath, $minified);
            }

            // ── Cache-busting version file ────────────────────
            // Pass the full script whitelist (built-in defaults + user
            // entries) so the early blocker skips them pre-consent.
            $this->writeVersionFile($websiteKey, $scriptPath, $cacheDisabled, $this->buildAllowedScripts($site, $websiteKey), $isIab === 1);
    ...
    private function loadBlockProviders(int $siteId, int $userId): array
    {
        ...
        try {
            ...
            foreach ($defaults as $row) {
                if (($row['provider_url'] ?? '') !== '') {
                    // The runtime blocker only acts on providers with a non-empty
                    // category match, so an empty list would never block anything.
                    $providers[] = [
                        'url' => $row['provider_url'],
                        'categories' => [!empty($row['default_category']) ? $row['default_category'] : 'marketing'],
                    ];
                }
            }
            ...
            $siteProviders = $this->db->fetchAllAssociative(
                'SELECT bp.provider_url, bp.default_category FROM oci_site_block_providers sbp
    ...
    private function normaliseContentSetting(array $content, string $cookieLaws): array
    {
        ...
        // but missing the gdpr/ccpa wrapper.
        // The frontend may also include flat keys alongside nested sections (mixed format),
        // so merge any flat keys into the appropriate nested sections before wrapping.
        if (isset($content['cookie_notice']) || isset($content['preference_center'])
            ...
            $noticeFlat = [
                ...
                'close_button', 'cookie_policy_label', 'cookie_policy_link',
                'disable_branding', 'custom_logo',
            ];
            foreach ($noticeFlat as $key) {
                if (\array_key_exists($key, $content) && !isset($content['cookie_notice'][$key])) {
                    $val = $content[$key];
                    if ($val) {
                        $mapped = $key === 'cookie_policy_link' ? 'cookie_policy_label' : $key;
                        $content['cookie_notice'][$mapped] = \is_bool($val) ? '1' : $val;
                    }
                    unset($content[$key]);
                }
            }

            $pcFlat = ['show_google_privacy_policy', 'respect_global_privacy_control', 'google_privacy_url'];
            foreach ($pcFlat as $key) {
                if (\array_key_exists($key, $content) && !isset($content['preference_center'][$key])) {
                    if ($content[$key]) {
                        $content['preference_center'][$key] = \is_bool($content[$key]) ? '1' : $content[$key];
                    }
                    unset($content[$key]);
                }
            }

            $clFlat = ['show_cookie_on_banner'];
            foreach ($clFlat as $key) {
                if (\array_key_exists($key, $content) && !isset($content['cookie_list'][$key])) {
                    if ($content[$key]) {
                        $content['cookie_list'][$key] = '1';
                    }
                    unset($content[$key]);
                }
            }

            $rcFlat = ['floating_button', 'button_position', 'revisit_custom_icon'];
            foreach ($rcFlat as $key) {
                if (\array_key_exists($key, $content)) {
                    $target = $key === 'revisit_custom_icon' ? 'custom_icon' : $key;
                    if (!isset($content['revisit_consent_button'][$target])) {
                        if ($content[$key]) {
                            $content['revisit_consent_button'][$target] = \is_bool($content[$key]) ? '1' : $content[$key];
                        }
                    }
                    unset($content[$key]);
                }
        ...
        try {
            return $this->db->fetchAllAssociative(
                'SELECT sc.id, sc.cookie_name, sc.cookie_domain, sc.category_id,
                 ...
                 WHERE sc.site_id = :sid AND sc.cookie_name <> \'\'
    ...
    private function loadBeacons(int $siteId): array
    {
        try {
            return $this->db->fetchAllAssociative(
                "SELECT id AS category_id, beacon_url AS url FROM oci_beacons WHERE site_id = :sid AND beacon_url <> ''",
                ['sid' => $siteId],
            );
        } catch (\Throwable) {
            return [];
    ...
    private function buildGdprNotice(
        ...
        if ($revisitButtonStatus) {
            ...
            if (($revisitConsentButton['style']['background'] ?? '') !== '') {
                $styles[] = 'background-color:' . $revisitConsentButton['style']['background'];
            }
    ...
    private function buildCcpaNotice(
        ...
        $globalPrivacyPolicy = $showRespectGpc
            ? '<div class="gpc-notice"><p>We respect the <a href="https://globalprivacycontrol.org/" target="_blank" rel="noopener noreferrer">Global Privacy Control</a> (GPC) signal. If your browser sends a GPC signal, we will treat it as a valid opt-out request.</p></div>'
            : '';
        ...
        $doNotSellCheckbox = '<div class="donotsell_checkbox_line"><input type="checkbox" class="cnz-custom-checkbox" value="1" id="donotsell_checkbox" name="donotsell_checkbox">&nbsp;&nbsp;<label for="donotsell_checkbox">' . $doNotSellLink . '</label></div>';

        // Opt out center buttons — fall back to sensible defaults when color settings are empty
        $oc = $colorSetting['opt_out_center'][$themeMode] ?? [];
        $saveBtnStyle = $oc['save_preference_button'] ?? [];
        if (empty($saveBtnStyle['background'])) {
            $saveBtnStyle = ['background' => '#115cfa', 'border' => '#115cfa', 'text' => '#ffffff'];
        }
        $cancelBtnStyle = $oc['cancel_button'] ?? [];
        if (empty($cancelBtnStyle['text'])) {
            $cancelBtnStyle = ['background' => '', 'border' => '#cccccc', 'text' => '#4b4b4b'];
        }
        $optOutCenter['buttons'] = [
            'save_preferences' => $this->buildButton(1, '[conzent_opt_out_center_save_preferences]', 'cookieOptSavePreferences', 'button', $saveBtnStyle),
            'cancel_button' => $this->buildButton(1, '[conzent_opt_out_center_cancel_button]', 'cookieOptCancel', 'link', $cancelBtnStyle),
        ];
    ...
    private function loadHtmlTemplate(string $cookieLaws, array $generalOption, array $bannerRow = [], int $siteId = 0): string
    {
        ...
        return $this->layoutService->renderForScript($html, [
            ...
            'opt_notice_buttons_html' => '[opt_notice_button_wrap]',
            'opt_center_buttons_html' => '[opt_center_button_wrap]',
            'do_not_sell_checkbox' => '[do_not_sell_checkbox]',
    ...
    private function buildCategoryHtml(array $cookieTypes): string
    {
        ...
        foreach ($cookieTypes as $val) {
            $slug = $val['value'] ?? '';
            // role="switch" + a localized aria-label give the bare checkbox an
            // accessible name; the placeholder pipeline resolves the category
            // title per language, so the label is always the visible title.
            if ($slug === 'necessary') {
                $btnText = '<div class="conzent-switch">'
                    . '<input type="checkbox" name="gdprPrefItem" value="necessary" class="sliding-switch" role="switch" aria-label="[conzent_preference_necessary_title]" checked="checked" disabled="disabled" data-compulsory="on">'
                    . '</div>';
            } else {
                $btnText = '<div class="conzent-switch">'
                    . "\n\t\t\t\t\t\t\t\t\t\t  <input type=\"checkbox\" id=\"gdprPrefItem{$slug}\" value=\"{$slug}\" name=\"gdprPrefItem\" class=\"sliding-switch\" role=\"switch\" aria-label=\"[conzent_preference_{$slug}_title]\" data-compulsory=\"off\">"
                    . "\n\t\t\t\t\t\t\t\t\t\t</div>";
            }
            $html .= "\n\t\t\t\t\t\t\t\t\t\t<div class=\"conzent-accordion\" id=\"cnzCategory{$slug}\">"
                ...
                . "\n\t\t\t\t\t\t\t\t\t\t\t\t<button type=\"button\" class=\"conzent-accordion-btn\" data-cookie-category=\"{$slug}\" aria-expanded=\"false\" aria-controls=\"cnzCategoryBody{$slug}\" style=\"color:[notice_description_color]!important;\">[conzent_preference_{$slug}_title]</button>"
                . "\n\t\t\t\t\t\t\t\t\t\t\t\t{$btnText}</div>"
                . "\n\t\t\t\t\t\t\t\t\t\t\t  <div class=\"conzent-accordion-header-des\" style=\"color:[notice_description_color]\">[conzent_preference_{$slug}_description]</div>"
                . "\n\t\t\t\t\t\t\t\t\t\t\t</div>"
                . "\n\t\t\t\t\t\t\t\t\t\t  </div>"
                . "\n\t\t\t\t\t\t\t\t\t\t  <div class=\"conzent-accordion-body\" id=\"cnzCategoryBody{$slug}\">"
    ...
    private function replaceHtmlPlaceholders(
        ...
        $optCenterButtons = $cookieLaws === 'ccpa'
            ? $this->renderButtonsHtml($optOutCenter['buttons'] ?? [])
            : '';
        $html = str_replace('[opt_center_button_wrap]', $optCenterButtons, $html);
        ...
        // Accessible name for the preference-center close button. English on
        // purpose: there is no translatable "close" field in the content
        // model, and a stable English aria-label beats a leaked placeholder.
        $html = str_replace('[conzent_close_label]', 'Close', $html);
    ...
    private function renderButtonsHtml(array $buttons): string
    {
        ...
        foreach ($buttons as $val) {
            ...
            if ($val['type'] === 'button') {
                $html .= '<button type="button" id="' . $val['tagId'] . '" style="' . $style . '" class="cnz-btn btn-' . $val['tagId'] . '">' . $val['label'] . '</button>';
            } elseif ($val['link'] === '') {
                // An empty link means the control is an action (close, cancel),
                // not navigation — a real <button>, not <a href="javascript:">,
                // so keyboard and screen-reader users get button semantics.
                $html .= '<button type="button" id="' . $val['tagId'] . '" style="' . $style . '" class="cnz-btn btn-' . $val['tagId'] . '">' . $val['label'] . '</button>';
            } else {
                $html .= '<a href="' . $val['link'] . '" id="' . $val['tagId'] . '" rel="nofollow" style="' . $style . '" class="cnz-btn btn-' . $val['tagId'] . '">' . $val['label'] . '</a>';
            }
        }
        return $html;
    }
    ...
    /**
     * Remove a leading "www." so a host can be used as a cookie domain.
     *
     * Deliberately a www-strip and NOT a registrable-domain calculation:
     * computing the true registrable domain needs the Public Suffix List
     * (getDomainOnly's "last two labels" turns example.co.uk into co.uk), and
     * the runtime guard already suffix-walks, so nothing here needs it.
     */
    private static function stripWwwPrefix(string $host): string
    {
        return (string) preg_replace('/^www\./i', '', trim($host));
    }
    ...
    /**
     * Merged script whitelist: built-in always-allowed patterns plus the
     * site's user-defined entries. Consumed by both the generated settings
     * (main blocker) and version.json (early blocker) so the two lists can
     * never drift apart.
     *
     ...
     * @return list<string>
     */
    private function buildAllowedScripts(array $site, string $websiteKey): array
    {
        $allowed = [
            'jquery.min.js', 'shopify.js', 'jquery.js', 'doofinder.com',
        ...
        if ($userAllowedRaw !== '') {
            ...
            if (\is_array($decoded)) {
                $allowed = array_merge($allowed, array_filter($decoded));
            }
        }
     ...
     */
    /**
     * @param list<string> $allowedScripts Full script whitelist (built-in defaults + user entries)
     * @param bool         $isIab          Site runs IAB TCF. The loader installs a
     *                                     __tcfapi stub on its first line so vendors
     *                                     find a CMP immediately, and tears it back
     *                                     down when this flag is absent — without it
     *                                     non-TCF sites would leave vendors waiting
     *                                     on a CMP that never loads.
     */
    private function writeVersionFile(string $websiteKey, string $scriptPath, bool $cacheDisabled = false, array $allowedScripts = [], bool $isIab = false): void
    {
        ...
        // Include the script whitelist so the early blocker can skip them
        if ($allowedScripts !== []) {
            $versionData['a'] = $allowedScripts;
        }

        // Tells the loader to keep its synchronous __tcfapi stub alive.
        if ($isIab) {
            $versionData['tcf'] = 1;
        }
    ...
    public function markSiteExceeded(string $websiteKey): void
    {
        ...
        // version.json is edge-cacheable for 60s — purge so the kill switch
        // takes effect immediately, not on cache expiry.
        $this->cachePurge->purgeForSite($websiteKey);
    }
    ...
    private function buildBannerConfig(array $optionsVal, array $bannerRow, array $ctx): array
    {
        ...
        $notice = [];
        // Flat OCI settings normalise to notice.*; legacy-nested settings carry the
        // flag under preference.* and bypass normalisation. A stored false is off.
        $categoryOnFirstLayer = !empty($layoutSetting['notice']['category_on_first_layer'])
            || !empty($layoutSetting['preference']['category_on_first_layer']) ? 1 : 0;
        ...
        $generalOption = [
            ...
            // Fail-open default: an unresolved geo lookup shows the banner
            // rather than silently running tracking on possibly-EU visitors.
            'geo_fail_open' => (int) (bool) ($generalSetting['geo_fail_open'] ?? 1),
            ...
            // GCM advanced signals — see setup_gcm_init in the runtime
            'url_passthrough' => (int) (bool) ($generalSetting['url_passthrough'] ?? 0),
            'ads_data_redaction' => (int) (bool) ($generalSetting['ads_data_redaction'] ?? 0),
            ...
            'cookie_samesite' => \in_array(strtolower((string) ($site['cookie_samesite'] ?? 'lax')), ['lax', 'strict', 'none'], true)
                ? strtolower((string) ($site['cookie_samesite'] ?? 'lax'))
                : 'lax',
            'advertiser_consent_mode' => (int) ($site['advertiser_consent_mode'] ?? 1),
        ];
        ...
        $allowedScripts = $this->buildAllowedScripts($site, $websiteKey);
        ...
        $htmlContent = $this->replaceHtmlPlaceholders(
            ...
            $googlePrivacyPolicy, $doNotSellCheckbox, $preferenceCenter, $optOutCenter
        );
        ...
        $configArray = [
            ...
            'cookiesList' => new \stdClass(), // Fetched at runtime via /api/v1/cookies
            'beaconsList' => $beacons,
            ...
            'close_button_status' => ($notice['buttons']['close_button']['status'] ?? 0),
     ...
     */
    /**
     * Strip every non-main-language translation out of a banner config.
     *
     * shortCodes[key][lang] and cookieTypes[i].{type,name,description}[lang]
     * are the per-language payloads; everything except the main language
     * moves into per-language side files. The runtime's existing fallback
     * (missing lang → main_lang) plus the boot-time hydration/aliasing keep
     * every read site working whether the side file loads or not.
     *
     * @return array{config: array<string, mixed>, i18n: array<string, array{sc?: array<string, mixed>, ct?: array<string, array<string, mixed>>}>}
     */
    private function splitTranslationsByLanguage(array $configArray, string $mainLang): array
    {
        $i18n = [];

        if (isset($configArray['shortCodes']) && \is_array($configArray['shortCodes'])) {
            foreach ($configArray['shortCodes'] as $key => $byLang) {
                if (!\is_array($byLang)) {
                    continue;
                }
                foreach ($byLang as $lang => $value) {
                    ...
                    $i18n[$lang]['sc'][$key] = $value;
                    unset($configArray['shortCodes'][$key][$lang]);
                }
            }
        }

        if (isset($configArray['cookieTypes']) && \is_array($configArray['cookieTypes'])) {
            foreach ($configArray['cookieTypes'] as $i => $entry) {
                $slug = (string) ($entry['value'] ?? $i);
                foreach (['type', 'name', 'description'] as $field) {
                    if (!isset($entry[$field]) || !\is_array($entry[$field])) {
                        continue;
                    }
                    foreach ($entry[$field] as $lang => $value) {
                        if ($lang === $mainLang) {
                            continue;
                        }
                        $i18n[$lang]['ct'][$slug][$field] = $value;
                        unset($configArray['cookieTypes'][$i][$field][$lang]);
                    }
                }
            }
        }

        return ['config' => $configArray, 'i18n' => $i18n];
    }

    /**
     * Write one lazy translation file per non-main language and return the
     * lang → URL map the runtime fetches from. URLs are content-hashed
     * (?v=) so they ride the immutable /sites_data/ cache rule — no purge
     * needed, a regeneration self-busts.
     *
     * @param array<string, array<string, mixed>> $i18nByLang
     * @return array<string, string>
     */
    private function writeI18nFiles(string $websiteKey, string $cookieLaws, array $i18nByLang): array
    {
        $map = [];
        foreach ($i18nByLang as $lang => $data) {
            if (!preg_match('/^[a-z]{2}(-[a-z0-9]{2,8})?$/i', (string) $lang)) {
                continue;
            }
            $json = json_encode($data, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
            $file = 'i18n-' . $cookieLaws . '-' . strtolower((string) $lang) . '.json';
            @file_put_contents($this->outputPath . '/' . $websiteKey . '/' . $file, $json);
            $map[$lang] = '[WEB_PATH]sites_data/' . $websiteKey . '/' . $file . '?v=' . substr(md5($json), 0, 10);
        }
    ...
    private function loadABVariants(
        ...
        foreach ($variants as $v) {
            ...
            // Build full config for this variant, slimmed the same way as the
            // control — otherwise every variant's full 86-language shortCodes
            // would diff against the control's slimmed one and explode the delta.
            $variantResult = $this->buildBannerConfig($variantOptionsVal, $variantBannerRow, $ctx);
            $variantConfig = $this->splitTranslationsByLanguage($variantResult['configArray'], $mainLang)['config'];
