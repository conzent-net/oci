...
final class CachePurgeService
{
    ...
    public function purgeForSite(string $websiteKey, string $domain = ''): void
    {
        ...
        // version.json is edge-cacheable (60s + stale windows) since the
        // offline-resilience work, so every regeneration and kill-switch
        // flip must purge it explicitly or the edge would serve the old
        // version hash for up to a minute.
        $appUrl = rtrim($_ENV['APP_URL'] ?? '', '/');
        $this->purgeCloudflareUrls([$appUrl . '/sites_data/' . $websiteKey . '/version.json']);
        $this->purgeOpcache();
        $this->purgeRedis($websiteKey);
    }

    /**
     * Purge the shared CMP loader and its companion statics from Cloudflare edge cache.
     */
