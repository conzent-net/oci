...
final class ConsentRepository implements ConsentRepositoryInterface
{
    ...
    public function incrementDailyStats(int $siteId, string $status, ?int $variantId, ?string $statDate = null): void
    {
        ...
        $this->db->executeStatement(
            'INSERT INTO oci_consent_daily_stats
             ...
             VALUES (:siteId, :statDate, :variantId, 1, :acc, :rej, :part)
    ...
    public function getConsentLogForExport(int $siteId, array $filters, int $afterId, int $batchSize): array
    {
        $qb = $this->db->createQueryBuilder()
            ->select(
                ...
                'c.tcf_data',
                'c.gacm_data',
                "COALESCE(GROUP_CONCAT(CONCAT(cc.category_slug, ':', cc.consent_status) ORDER BY cc.category_slug SEPARATOR '; '), '') AS categories",
            )
            ->from('oci_consents', 'c')
            ->leftJoin('c', 'oci_consent_categories', 'cc', 'cc.consent_id = c.id')
            ->where('c.site_id = :siteId')
            ->andWhere('c.id > :afterId')
            ->setParameter('siteId', $siteId, ParameterType::INTEGER)
            ->setParameter('afterId', $afterId, ParameterType::INTEGER)
            ->groupBy('c.id')
            ->orderBy('c.id', 'ASC')
    ...
    public function purgeOlderThan(int $days, int $batchSize = 5000): int
    {
        ...
        do {
            $deleted = (int) $this->db->executeStatement(
                'DELETE FROM oci_consents
                 WHERE consent_date < DATE_SUB(NOW(), INTERVAL :days DAY)
                 LIMIT :batch',
    ...
    /**
     * Filter variant for the export query, whose join makes the bare column
     * names ambiguous.
     *
     * @param \Doctrine\DBAL\Query\QueryBuilder $qb
     * @param array{status?: string, date_from?: string, date_to?: string} $filters
     */
    private function applyExportFilters(\Doctrine\DBAL\Query\QueryBuilder $qb, array $filters): void
    {
        if (!empty($filters['status'])) {
            $qb->andWhere('c.consent_status = :status')
                ->setParameter('status', $filters['status']);
        }

        if (!empty($filters['date_from'])) {
            $qb->andWhere('DATE(c.consent_date) >= :dateFrom')
                ->setParameter('dateFrom', $filters['date_from']);
        }

        if (!empty($filters['date_to'])) {
            $qb->andWhere('DATE(c.consent_date) <= :dateTo')
                ->setParameter('dateTo', $filters['date_to']);
        }
    }
}
