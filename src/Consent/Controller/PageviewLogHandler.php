...
final class PageviewLogHandler implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        // Offline-queue replay protection: a claimed event id is never
        // counted twice. Old bundles send no id and count as before. A
        // ledger failure processes without dedupe rather than dropping data.
        $eventId = ClientEventLedger::normalizeEventId($body['client_event_id'] ?? null);
        $occurredAt = ClientEventLedger::normalizeOccurredAt($body['occurred_at'] ?? null);
        if ($eventId !== null) {
            try {
                if (!$this->eventLedger->claim($siteId, $eventId, 'log', $occurredAt)) {
                    return $this->corsResponse(ApiResponse::json(['status' => 'ok'], 200));
                }
            } catch (\Throwable) {
                // Ledger unavailable — proceed without dedupe
            }
        }

        // Only count actual page loads as pageviews (not modal opens or post-consent re-scans)
        if ($requestType === 'banner_load') {
            // Replayed events count on the day the pageview happened, not
            // the day the server came back.
            $statDate = $occurredAt !== null ? substr($occurredAt, 0, 10) : date('Y-m-d');
