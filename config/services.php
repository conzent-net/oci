...
return [
    ...
    // ── AI client ───────────────────────────────────────────
    // Core default is a no-op; the Cloud Edition's Blog module overrides this
    // binding with the OpenRouter-backed client. Keeps core-domain services
    // (Banner/Cookie/Scanning) free of any cloud-module dependency.
    OCI\Infrastructure\Ai\AiClientInterface::class => autowire(OCI\Infrastructure\Ai\NullAiClient::class),

    // Replay dedupe for the banner's offline event queue
    OCI\Infrastructure\Ingest\ClientEventLedgerInterface::class => autowire(OCI\Infrastructure\Ingest\ClientEventLedger::class),
    ...
    OCI\Scanning\Repository\BeaconClassificationRepositoryInterface::class => autowire(OCI\Scanning\Repository\BeaconClassificationRepository::class),
    OCI\Scanning\Repository\BeaconReclassRequestRepositoryInterface::class => autowire(OCI\Scanning\Repository\BeaconReclassRequestRepository::class),
    ...
    // Backup heartbeat monitor: scalar basePath needs explicit wiring.
    OCI\Admin\Service\BackupFreshnessService::class => static function (ContainerInterface $c): OCI\Admin\Service\BackupFreshnessService {
        return new OCI\Admin\Service\BackupFreshnessService(
            $c->get(Doctrine\DBAL\Connection::class),
            $c->get(OCI\Identity\Service\MailerService::class),
            $c->get(Psr\Log\LoggerInterface::class),
            $c->get('config.base_path'),
        );
    },

    // Public status feed: scalar basePath needs explicit wiring.
    OCI\Admin\Service\StatusReportService::class => static function (ContainerInterface $c): OCI\Admin\Service\StatusReportService {
        return new OCI\Admin\Service\StatusReportService(
            $c->get(Doctrine\DBAL\Connection::class),
            $c->get(Predis\Client::class),
            $c->get(OCI\Scanning\Repository\ScanRepositoryInterface::class),
            $c->get(Psr\Log\LoggerInterface::class),
            $c->get('config.base_path'),
        );
    },
    OCI\Admin\Controller\StatusFeedHandler::class => static function (ContainerInterface $c): OCI\Admin\Controller\StatusFeedHandler {
        return new OCI\Admin\Controller\StatusFeedHandler($c->get('config.base_path'));
    },

    // Optional constructor params are NOT filled by plain autowiring — PHP-DI
    // leaves them at their default (null) unless wired explicitly. Without
    // this entry, site creation ran with no script generation, no logger and
    // no first-scan queueing. The Billing module overrides this entry and
    // must carry the same wiring plus subscriptionService.
    OCI\Site\Service\SiteCreationService::class => autowire(OCI\Site\Service\SiteCreationService::class)
        ->constructorParameter('scriptService', get(OCI\Banner\Service\ScriptGenerationService::class))
        ->constructorParameter('logger', get(Psr\Log\LoggerInterface::class))
        ->constructorParameter('scanService', get(OCI\Scanning\Service\ScanService::class)),
    ...
    OCI\Cookie\Repository\CookieReclassRequestRepositoryInterface::class => autowire(OCI\Cookie\Repository\CookieReclassRequestRepository::class),
    OCI\Cookie\Repository\CookieRegisterRepositoryInterface::class => autowire(OCI\Cookie\Repository\CookieRegisterRepository::class),
    ...
    // ── IAB Global Vendor List updater ──────────────────
    // TCF v2.4 disclosures (standardTexts, Feature illustrations, the renamed
    // Special Feature 2) are pure GVL data, so keeping this list fresh is the
    // primary compliance mechanism. Served statically from public/iab/gvl/.
    OCI\Compliance\Service\GvlHttpClientInterface::class => autowire(OCI\Compliance\Service\StreamGvlHttpClient::class),
    OCI\Compliance\Service\GvlUpdateService::class => static function (ContainerInterface $c): OCI\Compliance\Service\GvlUpdateService {
        return new OCI\Compliance\Service\GvlUpdateService(
            $c->get(LoggerInterface::class),
            $c->get(OCI\Compliance\Service\GvlHttpClientInterface::class),
            $c->get('config.base_path') . '/public/iab/gvl',
        );
    },
    // ── Self-serve account export (queue + status) ──────
    // Explicit factory: scalar base path is not autowired.
    OCI\Identity\Service\AccountExportJobService::class => static function (ContainerInterface $c): OCI\Identity\Service\AccountExportJobService {
        return new OCI\Identity\Service\AccountExportJobService(
            $c->get(Predis\Client::class),
            $c->get(Doctrine\DBAL\Connection::class),
            $c->get(OCI\Identity\Service\AccountExportService::class),
            $c->get(OCI\Admin\Service\AuditLogService::class),
            $c->get(LoggerInterface::class),
            (string) $c->get('config.base_path'),
        );
    },

    // ── Consent-log IP pseudonymization ─────────────────
    // Explicit factory: the DI container does not autowire scalar defaults,
    // and without the key the service silently degrades to truncation.
    OCI\Consent\Service\IpPseudonymizer::class => static function (ContainerInterface $c): OCI\Consent\Service\IpPseudonymizer {
        return new OCI\Consent\Service\IpPseudonymizer(
            trim((string) ($_ENV['CONSENT_IP_KEY'] ?? '')),
            $c->get(LoggerInterface::class),
        );
    },
    ...
    OCI\Identity\Service\EndpointrClient::class => static function (ContainerInterface $c): OCI\Identity\Service\EndpointrClient {
        return new OCI\Identity\Service\EndpointrClient(
            $c->get(LoggerInterface::class),
            $_ENV['ENDPOINTR_API_KEY'] ?? '',
        );
    },
