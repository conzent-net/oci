...
use OCI\Http\Handler\InstallPingHandler;
...
use OCI\Identity\Controller\RegisterVerifyPageHandler;
use OCI\Identity\Controller\StartVerificationHandler;
use OCI\Identity\Controller\VerifyCodeHandler;
use OCI\Identity\Controller\ResendCodeHandler;
use OCI\Identity\Controller\CancelVerificationHandler;
...
use OCI\Identity\Controller\AccountActivityHandler;
use OCI\Identity\Controller\AccountExportRequestHandler;
use OCI\Identity\Controller\AccountExportDownloadHandler;
...
use OCI\Scanning\Controller\BeaconClassifyHandler;
use OCI\Scanning\Controller\BeaconReclassRequestHandler;
...
use OCI\Scanning\Controller\ScanStatusHandler;
...
use OCI\Site\Controller\SiteVerifyHandler;
use OCI\Site\Controller\SiteSwitchHandler;
use OCI\Site\Controller\AssociatedDomainAddHandler;
use OCI\Site\Controller\AssociatedDomainListHandler;
use OCI\Site\Controller\AssociatedDomainRemoveHandler;
...
use OCI\Cookie\Controller\CookieClassifyHandler;
use OCI\Cookie\Controller\CookieReclassRequestHandler;
use OCI\Cookie\Controller\CookieChangesHandler;
use OCI\Cookie\Controller\CookieChangesAlertToggleHandler;
...
use OCI\Cookie\Controller\PublicCookieListHandler;
...
use OCI\Consent\Controller\ConsentExportHandler;
...
return static function (RouteCollector $r): void {
    ...
    // ── Public service-status feed (status page on getconzent.com) ──
    $r->get('/status.json', OCI\Admin\Controller\StatusFeedHandler::class);

    // ── Installer telemetry (public, no auth) ────────────
    $r->get('/ping', InstallPingHandler::class);
    ...
    $r->post('/register/start-verification', ['handler' => StartVerificationHandler::class, 'middleware' => 'guest']);
    $r->post('/register/verify-code', ['handler' => VerifyCodeHandler::class, 'middleware' => 'guest']);
    $r->post('/register/resend-code', ['handler' => ResendCodeHandler::class, 'middleware' => 'guest']);
    $r->post('/register/cancel-verification', ['handler' => CancelVerificationHandler::class, 'middleware' => 'guest']);
    $r->get('/register/verify', ['handler' => RegisterVerifyPageHandler::class, 'middleware' => 'guest']);
    ...
    $r->get('/account/activity', ['handler' => AccountActivityHandler::class, 'middleware' => 'web']);
    $r->get('/account/export/download', ['handler' => AccountExportDownloadHandler::class, 'middleware' => 'web']);
    $r->get('/app/account/export', ['handler' => AccountExportRequestHandler::class, 'middleware' => 'web']);
    $r->post('/app/account/export', ['handler' => AccountExportRequestHandler::class, 'middleware' => 'web']);
    ...
    $r->get('/sites/associated', ['handler' => AssociatedDomainListHandler::class, 'middleware' => 'web']);
    ...
    $r->get('/cookies/changes', ['handler' => CookieChangesHandler::class, 'middleware' => 'web']);
    ...
    $r->get('/consents/export', ['handler' => ConsentExportHandler::class, 'middleware' => 'web']);
    ...
    // ── A/B Tests ── routes live in the ABTest module (Cloud Edition only).
    ...
    $r->addGroup('/app/dashboard', static function (RouteCollector $r): void {
        ...
        // /app/dashboard/ab-summary is registered by the ABTest module (Cloud Edition).
    });

    // ── Public API (v1) ─────────────────────────────────
    $r->get('/api/v1/verify', SiteVerifyHandler::class);
    $r->get('/api/v1/geo_ip', GeoIpHandler::class);
    $r->get('/api/v1/cookies', PublicCookieListHandler::class);
    ...
    $r->addGroup('/app/scans', static function (RouteCollector $r): void {
        ...
        $r->get('/status', ['handler' => ScanStatusHandler::class, 'middleware' => 'web']);
    ...
    // ── Beacon AJAX API ────────────────────────────────────
    $r->addGroup('/app/beacons', static function (RouteCollector $r): void {
        $r->post('/classify', ['handler' => BeaconClassifyHandler::class, 'middleware' => 'web']);
        $r->post('/reclassify-request', ['handler' => BeaconReclassRequestHandler::class, 'middleware' => 'web']);
    });
    ...
    // ── Associated domains AJAX API ─────────────────────────
    $r->addGroup('/app/sites/associated', static function (RouteCollector $r): void {
        $r->post('/add', ['handler' => AssociatedDomainAddHandler::class, 'middleware' => 'web']);
        $r->post('/remove', ['handler' => AssociatedDomainRemoveHandler::class, 'middleware' => 'web']);
    });
    ...
    $r->addGroup('/app/cookies', static function (RouteCollector $r): void {
        ...
        $r->post('/classify', ['handler' => CookieClassifyHandler::class, 'middleware' => 'web']);
        $r->post('/reclassify-request', ['handler' => CookieReclassRequestHandler::class, 'middleware' => 'web']);
        $r->post('/changes-alerts', ['handler' => CookieChangesAlertToggleHandler::class, 'middleware' => 'web']);
    });
    ...
    // ── A/B Test AJAX API + Ad-platform integrations ──
    //    Registered by the ABTest module (Cloud Edition): /app/ab-tests/* and
    //    /app/integrations/* (Revenue Impact Tier 3).
