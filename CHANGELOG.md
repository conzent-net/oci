...
### Added
- **Email verification on signup** — New account registration now issues a 6-digit verification code (verify page, resend, and cancel flows) and tracks `email_verified` on users, with a daily `users:verify-emails` maintenance command
- **SMTP credential vault** — `MailerService` can pull SMTP host/port/user/pass from a named Endpointr vault entry, falling back to `MAIL_*` env vars when unavailable
- **Geo-blocking middleware** — `GeoBlockMiddleware` restricts access by country, resolving the real client IP from `CF-Connecting-IP` when behind Cloudflare
- **Public cookie list API** — New endpoint serves a site's cookie list (including observed cookies/beacons) from scan data, fetched lazily on preference-center open with cache-busting
- **Open Cookie Database sync** — `cookies:sync` imports and refreshes the global cookie reference database from the Open Cookie Database
- **Expanded language support** — Additional locales and pre-seeded cookie/banner translation data across supported languages
- **Site framework detection & verification** — Detect a site's CMS/framework and verify installation of the consent script
- **Install telemetry (opt-in phone-home)** — Anonymous install event tracking to help gauge self-hosted adoption

### Changed
- **Google Consent Mode v2** — Split GCM setup into standards-compliant `init` (sets `denied` defaults with `wait_for_update`) and post-interaction `update`; removed the non-standard `gtag('set', 'consent', …)` call
- **Geo IP behind Cloudflare** — Client IP resolution prefers `CF-Connecting-IP`; geo-IP responses are never Cloudflare-cached
- **Transactional email deliverability** — Reduced spam score on outgoing transactional mail

### Fixed
- **CCPA banner rendering** — Fixed CCPA banners inheriting the GDPR layout, unresolved shortcodes, GPC notice text, and the notice-bar close button (`#closeIcon`)
- **Duplicate banners** — Fixed a bug that could create duplicate banner records
- **Geo targeting** — Fixes to country-based banner targeting and selection
- **Language detection** — More reliable visitor language detection for banner content
