...
- **IAB TCF v2.4 Support** — Register your own CMP ID with IAB Europe and operate a fully certified TCF consent solution
...
Migrations seed all 86 supported languages; the translated content ships as
static caches and is loaded into the database with:

```bash
php bin/oci banners:seed-translations --insert   # banner texts, all languages
php bin/oci banners:seed-translations --categories  # cookie category names/descriptions
php bin/oci cookies:sync                         # pull the global cookie reference database
php bin/oci cookies:translate --insert           # cookie descriptions (~80 languages)
```

No AI key is needed — these read the caches shipped in `resources/data/` (`cookies:sync` fetches the Open Cookie Database from GitHub).
...
| **Compliance**   | IAB TCF v2.4, Google Consent Mode v2            |
...
After configuring your site and banner in the dashboard, add this one tag to your website's `<head>`, as early as
possible and before any other script:
...
That single tag is the whole install — everything is handled inside `consent.js`. It is deliberately parser-blocking:
do **not** add `async` or `defer`. Running synchronously is what guarantees that the Google Consent Mode defaults are
registered **before** any Google tag (gtag.js or GTM) initialises — a default that arrives later is silently
discarded, and the tag then behaves as if consent had been granted. The same ordering puts the IAB TCF stub in place
before ad vendors probe for it.

Because the defaults are pushed from inside the loader, they follow the `data-dl` attribute and land in a renamed GTM
dataLayer when you use one. The file is small and long-cached, so the cost is a single cached round trip, and it
handles all cookie consent UI, storage, and reporting automatically. See
[docs/embed-snippet.md](docs/embed-snippet.md) for the canonical snippet, including sites that use a renamed GTM
dataLayer.
...
4. The platform automatically enables TCF v2.4 features
